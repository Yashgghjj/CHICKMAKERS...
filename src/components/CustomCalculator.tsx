...
import type { Product, QuoteResult, CartItem, Mechanism } from '../types';
...
export default function CustomCalculator({
  ...
  const [productId, setProductId] = useState(preselectedProduct?.id || 'bamboo-chick-natural');
  ...
  return (
    <section id="calculator" className="py-16 bg-gradient-to-b from-brand-50 to-white">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          <Calculator className="w-10 h-10 text-brand-600 mx-auto mb-3" />
          <h2 className="font-display text-3xl font-bold text-sage-900 mb-2">Instant Price Calculator</h2>
          <p className="text-stone-500">Enter your exact dimensions for a precision quote</p>
        </div>

        <div className="bg-white rounded-2xl border border-stone-200 shadow-lg p-6 md:p-8">
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <select
                ...
                className="w-full border border-stone-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-brand-500 outline-none"
              >
                {PRODUCTS.map((p) => (
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              disabled={!quote}
              className="flex-1 flex items-center justify-center gap-2 bg-brand-600 text-white py-3 rounded-xl font-semibold hover:bg-brand-700 disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={!quote}
              className="flex-1 flex items-center justify-center gap-2 bg-sage-700 text-white py-3 rounded-xl font-semibold hover:bg-sage-900 disabled:opacity-50"
            >
              <Zap className="w-4 h-4" /> Buy Now
            </button>
            <button
              onClick={onBookMeasurement}
              className="flex items-center justify-center gap-2 border border-stone-300 px-6 py-3 rounded-xl hover:bg-stone-50"
            >
